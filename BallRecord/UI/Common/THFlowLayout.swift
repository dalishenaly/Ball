...
@objc protocol THCollectionViewFlowLayoutDelegate : NSObjectProtocol {
    /// 返回高度
    func th_setCellHeght(layout : THFlowLayout,indexPath : NSIndexPath,itemWidth : CGFloat) -> CGFloat
    
    /// 返回行数
    @objc optional func th_columnCountInWaterFlowLayout(layout : THFlowLayout) -> Int
    /// 返回列间距
    @objc optional func th_columnMarginInWaterFlowLayout(layout : THFlowLayout) -> CGFloat
    /// 返回行间距
    @objc optional func th_rowMarginInWaterFlowLayout(layout : THFlowLayout) -> CGFloat
    
    @objc optional func th_edgeInsetsInWaterFlowLayout(layout : THFlowLayout) -> UIEdgeInsets
}

class THFlowLayout: UICollectionViewFlowLayout {
    
    weak open var delegate : THCollectionViewFlowLayoutDelegate?
    ...
    fileprivate let defaultEdgeInsets : UIEdgeInsets = UIEdgeInsets(top: 10, left: 15, bottom: 10, right: 15)
    ...
    fileprivate func columnCount() -> Int { // 列数
        if self.delegate != nil && (self.delegate?.responds(to: #selector(self.delegate?.th_columnCountInWaterFlowLayout(layout:))))!{ // 判断代理是否实现 实现走代理方法
            return (self.delegate?.th_columnCountInWaterFlowLayout!(layout: self))!
        }else{ // 没有实现走默认的
    ...
    fileprivate func columnMargin() -> CGFloat {
        if self.delegate != nil && (self.delegate?.responds(to: #selector(self.delegate?.th_columnMarginInWaterFlowLayout(layout:))))!{
            return (self.delegate?.th_columnMarginInWaterFlowLayout!(layout: self))!
        }else{
    ...
    fileprivate func rowMargin() -> CGFloat {
        if self.delegate != nil && (self.delegate?.responds(to: #selector(self.delegate?.th_rowMarginInWaterFlowLayout(layout:))))!{
            return (self.delegate?.th_rowMarginInWaterFlowLayout!(layout: self))!
        }else{
    ...
    fileprivate func edgeInsets() -> UIEdgeInsets {
        if self.delegate != nil && (self.delegate?.responds(to: #selector(self.delegate?.th_edgeInsetsInWaterFlowLayout(layout:))))!{
            return (self.delegate?.th_edgeInsetsInWaterFlowLayout!(layout: self))!
        }else{
    ...
    override func prepare() {
        ...
        for i in 0 ..< count {
            ...
            //获取高度
            let cellHeight : CGFloat =  (self.delegate?.th_setCellHeght(layout: self, indexPath: indexPath as NSIndexPath, itemWidth: cellWidth))!
            /// 默认最小高度为第一组
