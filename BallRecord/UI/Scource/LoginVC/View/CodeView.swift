...
class CodeView: UIView {
    ...
    lazy var viewW:CGFloat = CGFloat()
    lazy var viewH:CGFloat = CGFloat()
...
extension CodeView {
    ...
    fileprivate func creatLineView() {
        for num in 0 ..< Int(Base.codeNum) {
            ...
            let lineView = UIView(frame: CGRect(x: x+x1, y: height -  Length.onceHeight.rawValue, width: Length.onceWidth.rawValue, height: 2))
    ...
    fileprivate func creatInputLabel() {
        for num in 0 ..< Int(Base.codeNum) {
            ...
            lineLayer.isHidden = num == 0 ? false : true
